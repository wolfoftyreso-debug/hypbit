...
/**
 * Declarative rule DSL evaluated by alert-engine.
 *
 * The alert-engine also computes a numeric `alert_score` for every
 * triggered alert using the formula below, then maps it to a severity:
 *
 *   alert_score = influence_score * 0.4
 *               + relevance_score * 0.4
 *               + impact_weight   * 0.2
 *
 *   impact_weight:  LOW=20  MEDIUM=60  HIGH=100
 *
...
export const AlertRuleSchema = z.object({
  ...
  // Optional override for severity mapping
