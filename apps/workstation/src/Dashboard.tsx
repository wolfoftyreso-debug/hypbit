...
const statusColor = (s: string) =>
  s === "GREEN" ? C.green : s === "YELLOW" ? C.yellow : s === "RED" ? C.red : C.tert;
const levelColor = (l: string) =>
  l === "L5" ? C.blue : l === "L4" ? C.green : l === "L3" ? C.yellow : l === "L2" ? C.orange : C.red;
const formatEur = (n: number) => `€${(n || 0).toLocaleString("sv-SE")}`;
const ncColor: Record<string, string> = { CRITICAL: C.red, MAJOR: C.orange, MINOR: C.yellow, OBSERVATION: C.tert };
...
const riskColor: Record<string, string> = { CRITICAL: C.red, HIGH: C.orange, MEDIUM: C.yellow, LOW: C.green };
...
const Card = ({ title, children, style: st }: { title?: string; children: React.ReactNode; style?: React.CSSProperties }) => (
  <div style={{ background: C.card, borderRadius: 12, padding: "20px 24px", boxShadow: shadow, ...st }}>
    {title && <div style={{ fontSize: 13, fontWeight: 600, color: C.sec, marginBottom: 16 }}>{title}</div>}
    {children}
  </div>
);
...
const KPI = ({ k }: { k: KPIData }) => (
  <div style={{ background: C.card, borderRadius: 12, padding: "14px 18px", boxShadow: shadow }}>
    <div style={{ display: "flex", justifyContent: "space-between" }}>
      <span style={{ fontSize: 12, color: C.sec }}>{k.name}</span>
      <span style={{ fontSize: 11, color: k.trend === "UP" ? C.green : C.tert }}>{k.trend === "UP" ? "↑" : "·"}</span>
    </div>
    <div style={{ fontSize: 26, fontWeight: 700, color: statusColor(k.status), marginTop: 4 }}>
      {k.unit === "EUR" ? formatEur(k.val) : k.val}{k.unit === "mån" ? " mån" : ""}
    </div>
    <div style={{ fontSize: 11, color: C.tert }}>Mål: {k.unit === "EUR" ? formatEur(k.target) : k.target}</div>
  </div>
);

const Dot = ({ level, size = 28 }: { level: string; size?: number }) => (
  <div style={{ width: size, height: size, borderRadius: 6, background: levelColor(level) + "22", color: levelColor(level), display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700 }}>{level}</div>
);
...
const Bar = ({ pct, color = C.blue }: { pct: number; color?: string }) => (
  <div style={{ flex: 1, height: 8, background: C.fill, borderRadius: 4, overflow: "hidden" }}>
...
export default function App() {
  ...
  // API data — falls back to FALLBACK if API is unavailable
  ...
  return (
    <div style={{ width: "100%", minHeight: "100vh", background: C.bg, fontFamily: "-apple-system, 'SF Pro Display', 'Helvetica Neue', sans-serif", color: C.text, WebkitFontSmoothing: "antialiased" }}>
      ...
      <div style={{ background: "rgba(255,255,255,0.72)", backdropFilter: "blur(20px)", borderBottom: `0.5px solid ${C.border}`, position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", height: 52 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ width: 24, height: 24, borderRadius: "50%", background: C.fill, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 600, color: C.sec }}>E</div>
            <span style={{ fontSize: 12, color: C.sec }}>{D.user.full_name}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "20px 20px 60px" }}>
              ...
              <Card title="Team">
                {D.team.map((t, i) => (
                  <Row key={i} border={i < 4}>
                    <div style={{ width: 32, height: 32, borderRadius: "50%", background: C.fill, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 600 }}>{t.name[0]}</div>
                    <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600 }}>{t.name}</div><div style={{ fontSize: 11, color: C.sec }}>{t.role}</div></div>
                    <span style={{ fontSize: 12, color: C.sec }}>{t.tasks}</span>
                    {t.overdue > 0 && <Badge color={C.yellow}>{t.overdue}</Badge>}
              ...
              <Card title="Beslut">
                {D.decisions.map((d, i) => (
                  <div key={i} style={{ padding: "10px 0", borderBottom: i < 2 ? `0.5px solid ${C.border}` : "none" }}>
                    ...
                    <div style={{ fontSize: 12, color: C.sec, marginTop: 2 }}>{d.rat}</div>
                    <div style={{ fontSize: 11, color: C.tert, marginTop: 2 }}>{d.by} · {d.date}</div>
                  </div>
                ))}
              </Card>
            </div>
        ...
        {view === "sales" && (
          <Card title="Pipeline">
            <div style={{ display: "flex", gap: 10 }}>
              {D.pipeline.map((p, i) => (
                <div key={i} style={{ flex: 1, background: C.fill, borderRadius: 10, padding: 14, textAlign: "center" }}>
                  <div style={{ fontSize: 10, color: C.sec, fontWeight: 600 }}>{p.st}</div>
                  <div style={{ fontSize: 22, fontWeight: 700, marginTop: 6 }}>{formatEur(p.eur)}</div>
                  <div style={{ fontSize: 12, color: C.sec }}>{p.deals}</div>
                </div>
              ))}
            </div>
          </Card>
        ...
        {/* FINANCE */}
        {view === "finance" && (
          <Card title={`Trial Balance ${D.tb.ok ? "✓" : "⚠"}`}>
            <div style={{ display: "grid", gridTemplateColumns: "80px 1fr 100px 100px", fontSize: 13 }}>
              {["Konto", "Namn", "Debet", "Kredit"].map(h => (
                <div key={h} style={{ padding: "8px 0", fontWeight: 600, color: C.sec, borderBottom: `0.5px solid ${C.border}`, textAlign: h === "Debet" || h === "Kredit" ? "right" : "left" }}>{h}</div>
              ))}
              {D.tb.rows.map((r, i) => [
                <div key={`c${i}`} style={{ padding: "10px 0", fontFamily: "monospace", borderBottom: `0.5px solid ${C.fill}` }}>{r.code}</div>,
                <div key={`n${i}`} style={{ padding: "10px 0", borderBottom: `0.5px solid ${C.fill}` }}>{r.name}</div>,
                <div key={`d${i}`} style={{ padding: "10px 0", textAlign: "right", fontFamily: "monospace", color: r.d ? C.text : C.tert, borderBottom: `0.5px solid ${C.fill}` }}>{r.d ? formatEur(r.d) : "—"}</div>,
                <div key={`k${i}`} style={{ padding: "10px 0", textAlign: "right", fontFamily: "monospace", color: r.c ? C.text : C.tert, borderBottom: `0.5px solid ${C.fill}` }}>{r.c ? formatEur(r.c) : "—"}</div>,
              ])}
              <div /><div style={{ padding: "12px 0", fontWeight: 700 }}>Totalt</div>
              <div style={{ padding: "12px 0", fontWeight: 700, textAlign: "right", fontFamily: "monospace" }}>{formatEur(D.tb.d)}</div>
              <div style={{ padding: "12px 0", fontWeight: 700, textAlign: "right", fontFamily: "monospace" }}>{formatEur(D.tb.c)}</div>
            </div>
            {D.tb.ok && <div style={{ marginTop: 12, fontSize: 13, color: C.green, fontWeight: 600 }}>✓ Balanserad</div>}
          </Card>
        )}
        ...
        {view === "tasks" && (
          <Card>
            {D.tasks.sort((a, b) => a.p - b.p).map((t, i) => (
              <Row key={i} border={i < D.tasks.length - 1}>
                <div style={{ width: 4, height: 32, borderRadius: 2, background: t.p === 1 ? C.red : t.p === 2 ? C.blue : C.tert }} />
                <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600 }}>{t.title}</div><div style={{ fontSize: 12, color: C.sec }}>{t.who}</div></div>
                <Badge color={t.st === "IN_PROGRESS" ? C.blue : C.tert}>{t.st === "IN_PROGRESS" ? "Pågående" : "Att göra"}</Badge>
                <span style={{ fontSize: 12, color: C.sec, fontFamily: "monospace" }}>{t.dl}</span>
              </Row>
            ))}
          </Card>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <Card title="Teamöversikt">
              <div style={{ overflowX: "auto" }}>
                <div style={{ display: "grid", gridTemplateColumns: "160px repeat(5, 1fr)", gap: 0, fontSize: 12 }}>
                  <div style={{ padding: 8, fontWeight: 600, color: C.sec }}>Capability</div>
                  {["Erik", "Leon", "Johan", "Dennis", "Winston"].map(n => (
                    <div key={n} style={{ padding: 8, fontWeight: 600, color: C.sec, textAlign: "center" }}>{n}</div>
                  ))}
                  {D.heatmap.map((r, i) => [
                    <div key={`n${i}`} style={{ padding: 8, borderTop: `0.5px solid ${C.fill}` }}><span style={{ fontWeight: 500 }}>{r.cap}</span><br /><span style={{ fontSize: 10, color: C.tert }}>{r.dom}</span></div>,
                    ...["E", "L", "J", "Dn", "W"].map(p => (
                      <div key={`${i}${p}`} style={{ padding: 8, borderTop: `0.5px solid ${C.fill}`, display: "flex", justifyContent: "center" }}>
            ...
            <Card title="Gaps">
              {D.gaps.map((g, i) => (
                <Row key={i} border={i < D.gaps.length - 1}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: C.fill, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 600 }}>{g.who[0]}</div>
                  <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600 }}>{g.who}</div><div style={{ fontSize: 12, color: C.sec }}>{g.cap}</div></div>
                  <Dot level={g.cur} /><span style={{ color: C.tert }}>→</span><Dot level={g.tgt} />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {D.devPlans.map((p, pi) => (
              <Card key={pi} title={`${p.who} — Utvecklingsplan`}>
                {p.actions.map((a, i) => (
                  <Row key={i} border={i < p.actions.length - 1}>
                    <div style={{ width: 4, height: 32, borderRadius: 2, background: a.st === "ACTIVE" ? C.blue : C.tert }} />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 14, fontWeight: 600 }}>{a.title}</div>
                      <div style={{ display: "flex", gap: 6, marginTop: 4 }}><Badge color={C.purple}>{a.cap}</Badge><Badge color={C.tert}>{a.type}</Badge></div>
                    </div>
                    <Badge color={a.st === "ACTIVE" ? C.blue : C.tert}>{a.st === "ACTIVE" ? "Pågående" : "Väntande"}</Badge>
                    <span style={{ fontSize: 12, color: C.sec, fontFamily: "monospace" }}>{a.dl}</span>
                  </Row>
                ))}
              </Card>
            ))}
            ...
            {D.goals.map((g, i) => (
              <Card key={i}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                  ...
                  <Badge color={g.st === "ON_TRACK" ? C.green : C.blue}>{g.st.replace("_", " ")}</Badge>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 16 }}>
                  <Bar pct={(g.cur / g.tgt) * 100} color={g.cur >= g.tgt ? C.green : C.blue} />
                  <span style={{ fontSize: 15, fontWeight: 700, minWidth: 60 }}>{g.cur}/{g.tgt}</span>
                  <span style={{ fontSize: 12, color: C.sec }}>{g.unit}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12 }}>
                  <span style={{ fontSize: 12, color: C.sec }}>Deadline: {g.end}</span>
                  <span style={{ fontSize: 12, color: C.sec }}>Readiness: <span style={{ fontWeight: 600, color: g.ready >= 70 ? C.green : g.ready >= 40 ? C.yellow : C.red }}>{g.ready}%</span></span>
                </div>
              </Card>
            ))}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
              {[
                ...
                { l: "Körningar / 30d", v: D.processes.reduce((s, p) => s + p.runs30d, 0), c: C.green },
                { l: "Snitt duration", v: D.processes.length > 0 ? Math.round(D.processes.reduce((s, p) => s + p.avgMin, 0) / D.processes.length) + " min" : "—", c: C.sec },
              ].map((s, i) => (
                <div key={i} style={{ background: C.card, borderRadius: 12, padding: "16px 20px", boxShadow: shadow }}>
                  <div style={{ fontSize: 12, color: C.sec }}>{s.l}</div>
            ...
            <Card title="Processregister">
              {D.processes.map((p, i) => (
                <Row key={i} border={i < D.processes.length - 1}>
                  <div style={{ minWidth: 80, fontSize: 12, fontFamily: "monospace", color: C.sec }}>{p.code}</div>
                  <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600 }}>{p.name}</div><div style={{ fontSize: 12, color: C.sec }}>Ägare: {p.owner}</div></div>
                  <span style={{ fontSize: 12, color: C.sec }}>{p.runs30d} körn.</span>
                  <span style={{ fontSize: 12, color: C.sec }}>{p.avgMin} min</span>
                  {p.ncs > 0 ? <Badge color={C.yellow}>{p.ncs} NC</Badge> : <Badge color={C.green}>0 NC</Badge>}
                </Row>
              ))}
            </Card>
          </div>
        ...
        {view === "nc" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
              {[
                { l: "Öppna", v: D.ncs.filter(n => n.status !== "CLOSED").length, c: C.red },
                { l: "Stängda", v: D.ncs.filter(n => n.status === "CLOSED").length, c: C.green },
                { l: "Snitt dagar", v: D.ncs.length > 0 ? Math.round(D.ncs.reduce((s, n) => s + n.days, 0) / D.ncs.length) : 0, c: C.sec },
                { l: "MAJOR+", v: D.ncs.filter(n => n.severity === "MAJOR" || n.severity === "CRITICAL").length, c: C.orange },
              ].map((s, i) => (
                <div key={i} style={{ background: C.card, borderRadius: 12, padding: "16px 20px", boxShadow: shadow }}>
                  <div style={{ fontSize: 12, color: C.sec }}>{s.l}</div>
                  <div style={{ fontSize: 26, fontWeight: 700, color: s.c, marginTop: 4 }}>{s.v}</div>
            ...
            <Card title="Avvikelser">
              {D.ncs.map((n, i) => (
                <Row key={i} border={i < D.ncs.length - 1}>
                  <div style={{ minWidth: 90, fontSize: 12, fontFamily: "monospace", color: C.sec }}>{n.code}</div>
                  <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600 }}>{n.title}</div><div style={{ fontSize: 12, color: C.sec }}>{n.who} · {n.days}d</div></div>
                  <Badge color={ncColor[n.severity]}>{n.severity}</Badge>
                  <Badge color={n.status === "CLOSED" ? C.green : C.blue}>{ncStatusLabel[n.status] ?? n.status}</Badge>
                </Row>
              ))}
            </Card>
          </div>
        ...
        {view === "improvements" && (
          <Card title="Förbättringar (PDCA)">
            {D.improvements.map((imp, i) => (
              <Row key={i} border={i < D.improvements.length - 1}>
                <div style={{ minWidth: 100, fontSize: 12, fontFamily: "monospace", color: C.sec }}>{imp.code}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>{imp.title}</div>
                  <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                    <Badge color={C.purple}>{imp.pdca}</Badge>
                    <span style={{ fontSize: 11, color: C.sec }}>Impact: {imp.impact} · Effort: {imp.effort}</span>
                  </div>
                </div>
                <Badge color={imp.status === "IMPLEMENTING" ? C.blue : imp.status === "APPROVED" ? C.green : C.tert}>{imp.status}</Badge>
              </Row>
            ))}
          </Card>
        )}
        ...
        {view === "compliance" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {D.compliance.map((c, i) => (
              <Card key={i}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                  <div style={{ fontSize: 17, fontWeight: 700 }}>{c.standard}</div>
                  <Badge color={c.pct >= 90 ? C.green : c.pct >= 70 ? C.yellow : C.red}>{c.pct}%</Badge>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 16 }}>
                  <Bar pct={c.pct} color={c.pct >= 90 ? C.green : c.pct >= 70 ? C.yellow : C.red} />
                  <span style={{ fontSize: 15, fontWeight: 700 }}>{c.ok}/{c.total}</span>
                </div>
                <div style={{ display: "flex", gap: 16, marginTop: 12, fontSize: 12, color: C.sec }}>
                  <span>✓ {c.ok} uppfyllda</span>
                  <span>◐ {c.partial} partiella</span>
                  <span>✕ {c.fail} ej uppfyllda</span>
                  <span>? {c.na} ej bedömda</span>
                  {c.audit && <span>Revision: {c.audit}</span>}
                </div>
              </Card>
            ))}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
              {["CRITICAL", "HIGH", "MEDIUM", "LOW"].map(l => {
                ...
                return (
                  <div key={l} style={{ background: C.card, borderRadius: 12, padding: "16px 20px", boxShadow: shadow, borderTop: `3px solid ${riskColor[l]}` }}>
                    <div style={{ fontSize: 12, color: C.sec }}>{l}</div>
                    <div style={{ fontSize: 26, fontWeight: 700, color: riskColor[l], marginTop: 4 }}>{count}</div>
                  </div>
                );
              })}
            </div>
            <Card title="Riskregister">
              {D.risks.sort((a, b) => b.score - a.score).map((r, i) => (
                <Row key={i} border={i < D.risks.length - 1}>
                  <div style={{ minWidth: 80, fontSize: 12, fontFamily: "monospace", color: C.sec }}>{r.code}</div>
                  <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600 }}>{r.title}</div><div style={{ fontSize: 12, color: C.sec }}>{r.mit}</div></div>
                  <Badge color={C.tert}>{r.cat}</Badge>
                  <div style={{ textAlign: "center", minWidth: 40 }}>
                    <div style={{ fontSize: 16, fontWeight: 700, color: riskColor[r.level] }}>{r.score}</div>
                    <div style={{ fontSize: 9, color: C.tert }}>{r.prob}×{r.imp}</div>
                  </div>
        ...
        {view === "chat" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12, height: "calc(100vh - 120px)" }}>
            <div style={{ display: "flex", gap: 6 }}>
              {["SALES", "PRODUCT", "OPS", "FINANCE", "DECISIONS"].map(ch => (
                <button key={ch} onClick={() => setChatCh(ch)} style={{ background: chatCh === ch ? C.blue : C.card, color: chatCh === ch ? "#FFF" : C.sec, border: `0.5px solid ${chatCh === ch ? C.blue : C.border}`, borderRadius: 20, padding: "6px 14px", fontSize: 12, fontWeight: 500, cursor: "pointer" }}>#{ch.toLowerCase()}</button>
              ))}
            </div>
            <div style={{ flex: 1, background: C.card, borderRadius: 12, boxShadow: shadow, padding: 20, overflowY: "auto", display: "flex", flexDirection: "column-reverse", gap: 14 }}>
              {D.msgs.filter(m => m.ch === chatCh).map((m, i) => (
                <div key={i} style={{ display: "flex", gap: 10 }}>
                  <div style={{ width: 30, height: 30, borderRadius: "50%", background: m.s ? C.blue + "15" : C.fill, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 600, color: m.s ? C.blue : C.text, flexShrink: 0 }}>{m.s ? "⚙" : m.u[0]}</div>
                  <div><div style={{ fontSize: 13, fontWeight: 600, color: m.s ? C.blue : C.text }}>{m.u}</div><div style={{ fontSize: 13, color: C.text, marginTop: 2 }}>{m.t}</div></div>
                </div>
              ))}
              {D.msgs.filter(m => m.ch === chatCh).length === 0 && <div style={{ textAlign: "center", color: C.tert, padding: 40 }}>Tomt</div>}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <input type="text" placeholder={`#${chatCh.toLowerCase()}`} style={{ flex: 1, background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", fontSize: 13, color: C.text, outline: "none", boxShadow: shadow }} />
