...
const categoryColor: Record<string, string> = {
  market: '#2563eb', security: '#dc2626', competitor: '#d97706', opportunity: '#16a34a', risk: '#7c3aed',
}
  ...
  return (
    <div>
      <div style={{ background: 'var(--color-brand)', borderRadius: 12, padding: '24px 28px', marginBottom: 24, color: 'var(--color-text-inverse)' }}>
        <div style={{ fontSize: 9, fontFamily: 'monospace', color: 'var(--color-accent)', letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 8 }}>Wavult Intelligence</div>
        <h2 style={{ fontSize: 22, fontWeight: 800, margin: '0 0 6px' }}>System Intelligence</h2>
        <p style={{ fontSize: 13, color: 'rgba(245,240,232,.6)', margin: 0 }}>Signaler, mönster och insikter från hela systemet</p>
      </div>

      {loading && [1,2,3].map(i => <div key={i} style={{ background: 'var(--color-bg-muted)', borderRadius: 10, height: 80, marginBottom: 10, animation: 'pulse 1.5s ease-in-out infinite' }} />)}
      ...
      {!loading && !error && signals.length === 0 && (
        <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 12, padding: '64px 24px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--color-text-muted)', maxWidth: 320, margin: '0 auto' }}>Systemet analyserar kontinuerligt och rapporterar insikter här när de uppstår</div>
        </div>
      )}
      ...
      {!loading && !error && signals.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {signals.map(s => (
            <div key={s.id} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 12, padding: '18px 20px', borderLeft: `4px solid ${categoryColor[s.category] ?? 'var(--color-accent)'}` }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--color-brand)' }}>{s.confidence}%</div>
