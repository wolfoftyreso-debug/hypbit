    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-[#0D0F1A] border border-white/10 rounded-xl w-full max-w-lg">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
          <h2 className="text-white font-semibold">Ny audience</h2>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Geografi (kommaseparerat)</label>
            <input
              value={form.geo}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-gray-400 mb-1">Ålder — från</label>
              <input
                type="number"
                value={form.age_min}
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">Ålder — till</label>
              <input
                type="number"
                value={form.age_max}
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Intressen (kommaseparerat)</label>
            <input
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">CRM-segment (valfritt)</label>
            <input
          ...
          <button
            disabled
            className="w-full py-2 text-sm text-gray-600 border border-white/5 rounded-lg cursor-not-allowed"
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-white font-semibold">Audiences</h2>
          <p className="text-xs text-gray-500 mt-0.5">{MOCK_AUDIENCES.length} audiences definierade</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            disabled
            className="px-3 py-1.5 text-sm text-gray-600 border border-white/5 rounded-lg cursor-not-allowed"
            title="Aktiveras i Fas 2"
          >
            📥 Importera från CRM
          </button>
      ...
      <div className="space-y-2">
        {MOCK_AUDIENCES.map(audience => {
          ...
          return (
            <div
              key={audience.id}
              className="bg-[#0D0F1A] border border-white/10 rounded-xl p-4"
            >
      ...
      <div className="rounded-lg bg-gray-900/50 border border-white/10 px-4 py-3 text-xs text-gray-500">
