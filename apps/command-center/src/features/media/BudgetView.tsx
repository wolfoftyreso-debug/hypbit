  ...
  return (
    <div className="bg-[#0D0F1A] border border-white/10 rounded-xl p-5 space-y-4">
        ...
        {allocations.map(alloc => {
          ...
          return (
            <div key={alloc.id} className="flex items-center gap-3 py-2 border-b border-white/5 last:border-0">
              <span className="text-xs text-gray-300 w-32 flex-shrink-0">{ch?.provider ?? alloc.channel_id}</span>
              ...
              <input
                ...
                className="w-20 bg-[#070912] border border-white/10 rounded px-2 py-1 text-xs text-white font-mono text-right focus:outline-none focus:border-brand-accent/30"
              />
              <span className="text-xs text-gray-600 w-10">{campaign.currency}/d</span>
            </div>
          )
        })}
      </div>
      ...
      <div className="flex items-center justify-between pt-1">
        <div className="flex items-center gap-2">
          <div className="w-8 h-4 rounded-full bg-gray-700 flex items-center px-0.5 cursor-not-allowed" title="Aktiveras i Fas 3">
            <div className="w-3 h-3 rounded-full bg-gray-500" />
          </div>
          <span className="text-xs text-gray-600">Auto-optimize — Fas 3</span>
        </div>
...
export function BudgetView() {
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-white font-semibold">Budgethantering</h2>
