    ...
    <div className="bg-white rounded-xl p-6 text-gray-900 min-w-[400px] shadow-2xl">
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="text-right">
          ...
          {fe?.orgNr && <p className="text-xs text-gray-500">Org.nr: {fe.orgNr}</p>}
          <p className="text-xs text-gray-500">{fe?.jurisdiction}</p>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
        <div>
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Mottagare</p>
        ...
        <tbody>
          {invoice.lines.map((line, i) => (
            <tr key={i} className="border-b border-gray-100">
              ...
              <td className="py-2 text-right text-gray-500">{line.qty}</td>
              <td className="py-2 text-right text-gray-500">{line.unitPrice.toLocaleString()}</td>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-gray-900">Fakturor</h2>
        ...
        {(['all', 'draft', 'sent', 'paid', 'overdue'] as const).map(s => {
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors flex items-center gap-1.5 ${statusFilter === s ? 'bg-white/10 text-gray-900' : 'text-gray-500 hover:text-gray-500'}`}
            >
      ...
      {showNewForm && (
        <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
          <h3 className="text-sm font-bold text-gray-900">Ny faktura</h3>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-gray-200">
            ...
            <span className="text-[14px] font-bold text-gray-900">{lineTotal.toLocaleString()} {form.currency}</span>
          </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
        ...
        <div className="grid grid-cols-12 px-4 py-2 text-[9px] font-mono text-gray-500 uppercase tracking-wider border-b border-gray-200 min-w-[560px]">
          ...
          return (
            <div key={inv.id}
              ...
              <div className="col-span-3">
                ...
                <p className="text-[9px] text-gray-500 truncate">{inv.recipientEmail}</p>
              </div>
              ...
              <span className="col-span-2 text-right text-xs font-mono font-semibold text-gray-900">
              ...
              <div className="col-span-1 flex justify-end">
                <button
                  ...
                  className="text-xs text-gray-500 hover:text-gray-900 transition-colors px-2 py-1 rounded bg-white/[0.04] hover:bg-white/[0.08]"
                >
                  Preview
                </button>
              </div>
