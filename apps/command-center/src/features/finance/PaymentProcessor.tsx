  ...
  return (
    <div className="bg-[#0D0F1A] rounded-xl border border-brand-accent/20 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h4 className="text-[12px] font-semibold text-white">Payment Intent</h4>
      ...
      {intent.metadata && (
        <div>
          <span className="text-[10px] text-gray-600 font-mono uppercase tracking-wider block mb-1">Metadata</span>
      ...
      {canRefund && (
        <button
          ...
          className="w-full px-3 py-2 rounded-lg text-[11px] font-medium bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 transition-colors"
        >
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-[#0D0F1A] rounded-xl border border-green-500/20 p-3">
          <div className="text-[10px] text-gray-500 mb-1">Avvecklat (SEK)</div>
        ...
        <div className="bg-[#0D0F1A] rounded-xl border border-blue-500/20 p-3">
          <div className="text-[10px] text-gray-500 mb-1">Under behandling</div>
        ...
        <div className="bg-[#0D0F1A] rounded-xl border border-red-500/20 p-3">
          <div className="text-[10px] text-gray-500 mb-1">Misslyckade</div>
      ...
      <div className="flex gap-2 flex-wrap">
        <div className="flex items-center gap-1 p-0.5 bg-[#0D0F1A] rounded-lg border border-white/[0.06]">
          <button
            ...
            className={`px-2 py-1 rounded-md text-[10px] font-medium transition-colors ${statusFilter === 'all' ? 'bg-brand-accent/20 text-brand-accent' : 'text-gray-500 hover:text-gray-300'}`}
          >
            Alla
          </button>
          {(['CREATED', 'PROCESSING', 'AUTHORIZED', 'CAPTURED', 'SETTLED', 'FAILED', 'REFUNDED'] as PaymentStatus[]).map(s => (
            <button
              ...
              className={`px-2 py-1 rounded-md text-[10px] font-medium transition-colors ${statusFilter === s ? `${PAYMENT_STATUS_CONFIG[s].bg} ${PAYMENT_STATUS_CONFIG[s].color}` : 'text-gray-500 hover:text-gray-300'}`}
            >
        ...
        <div className="flex items-center gap-1 p-0.5 bg-[#0D0F1A] rounded-lg border border-white/[0.06]">
          {(['all', 'revolut', 'stripe'] as const).map(p => (
            <button
              ...
              className={`px-2 py-1 rounded-md text-[10px] font-medium transition-colors ${pspFilter === p ? 'bg-brand-accent/20 text-brand-accent' : 'text-gray-500 hover:text-gray-300'}`}
            >
        ...
        {STATUS_ORDER.map((s, i) => {
          ...
          return (
            <div key={s} className="flex items-center gap-1 flex-shrink-0">
              <div className={`px-2.5 py-1 rounded-lg border text-[10px] font-medium ${sc.bg} ${sc.color}`}>
                {sc.label} ({count})
              </div>
      ...
      <div className="bg-[#0D0F1A] rounded-xl border border-white/[0.06] overflow-hidden">
        <div className="grid grid-cols-[1fr_100px_120px_90px_80px] gap-3 px-4 py-2.5 border-b border-white/[0.06] text-[10px] text-gray-600 font-mono uppercase tracking-wider">
        ...
        <div className="divide-y divide-white/[0.04]">
          {filtered.map(intent => {
            ...
            return (
              ...
              >
                <div>
                  <div className="text-[12px] text-gray-200 truncate">{intent.description}</div>
                  <div className="text-[10px] text-gray-600">{intent.entity}</div>
                </div>
                <div className="flex flex-col justify-center">
                  <span className="text-[12px] font-semibold text-white">
                    {new Intl.NumberFormat('sv-SE').format(intent.amount / 100)}
                  </span>
                  <span className="text-[10px] text-gray-600">{intent.currency}</span>
                </div>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-[#0D0F1A] rounded-xl border border-blue-500/20 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              <h4 className="text-[13px] font-bold text-white">Revolut Business</h4>
              <p className="text-[10px] text-gray-500">Intercompany + EUR/AED</p>
            </div>
            <div className="ml-auto flex items-center gap-1.5 px-2 py-1 rounded-lg bg-green-500/10 border border-green-500/20 text-[10px] text-green-400">
          ...
          <div className="space-y-2">
            {PSP_RULES.filter(r => r.psp === 'revolut').map((rule, i) => (
              <div key={i} className="bg-[#07080F] rounded-lg p-2.5">
                <div className="text-[11px] font-medium text-blue-300 mb-1">{rule.condition}</div>
                <div className="text-[10px] text-gray-500 mb-1.5">{rule.reason}</div>
        ...
        <div className="bg-[#0D0F1A] rounded-xl border border-purple-500/20 p-4">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xl">⚡</span>
            <div>
              <h4 className="text-[13px] font-bold text-white">Stripe</h4>
              <p className="text-[10px] text-gray-500">Kundbetalningar + fakturering</p>
            </div>
            <div className="ml-auto flex items-center gap-1.5 px-2 py-1 rounded-lg bg-green-500/10 border border-green-500/20 text-[10px] text-green-400">
              <span className="h-1.5 w-1.5 rounded-full bg-green-400" />
              Aktiv
          ...
          <div className="space-y-2">
            {PSP_RULES.filter(r => r.psp === 'stripe').map((rule, i) => (
              <div key={i} className="bg-[#07080F] rounded-lg p-2.5">
                <div className="text-[11px] font-medium text-purple-300 mb-1">{rule.condition}</div>
                <div className="text-[10px] text-gray-500 mb-1.5">{rule.reason}</div>
                <div className="flex flex-wrap gap-1">
                  {rule.examples.map(ex => (
      ...
      <div className="bg-[#0D0F1A] rounded-xl border border-white/[0.06] overflow-hidden">
        <div className="px-4 py-3 border-b border-white/[0.06]">
          <h4 className="text-[12px] font-semibold text-white">Regelmatris — Belopp / Valuta / Typ → PSP</h4>
        </div>
        <div className="grid grid-cols-[1fr_80px_1fr_80px] gap-0">
          <div className="px-4 py-2 bg-white/[0.03] text-[10px] text-gray-600 font-mono uppercase tracking-wider border-b border-white/[0.06]">Typ</div>
          <div className="px-4 py-2 bg-white/[0.03] text-[10px] text-gray-600 font-mono uppercase tracking-wider border-b border-white/[0.06]">Valuta</div>
          <div className="px-4 py-2 bg-white/[0.03] text-[10px] text-gray-600 font-mono uppercase tracking-wider border-b border-white/[0.06]">Belopp</div>
          <div className="px-4 py-2 bg-white/[0.03] text-[10px] text-gray-600 font-mono uppercase tracking-wider border-b border-white/[0.06]">PSP</div>
          {[
            ...
            <>
              <div key={`${i}-typ`} className={`px-4 py-2.5 text-[11px] text-gray-300 border-b border-white/[0.04] ${i % 2 === 0 ? '' : 'bg-white/[0.01]'}`}>{row.typ}</div>
              <div key={`${i}-val`} className={`px-4 py-2.5 text-[11px] text-gray-400 font-mono border-b border-white/[0.04] ${i % 2 === 0 ? '' : 'bg-white/[0.01]'}`}>{row.valuta}</div>
              <div key={`${i}-bel`} className={`px-4 py-2.5 text-[11px] text-gray-400 border-b border-white/[0.04] ${i % 2 === 0 ? '' : 'bg-white/[0.01]'}`}>{row.belopp}</div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-[11px] text-yellow-400">
          Att betala ut: {new Intl.NumberFormat('sv-SE').format(totalPending)} SEK/EUR
        </div>
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#0D0F1A] border border-white/[0.06] text-[11px] text-gray-400">
              ...
              <div className="flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3 mb-1">
                    <span className="text-[12px] font-medium text-gray-200 leading-tight">{payout.recipient}</span>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border flex-shrink-0 ${sc.bg} ${sc.color}`}>
                      {sc.label}
                    </span>
                  </div>
                  <p className="text-[11px] text-gray-500 mb-2">{payout.description}</p>
                ...
                {payout.status === 'pending' && isAdmin && (
                  <button
                    ...
                    className="flex-shrink-0 px-3 py-1.5 rounded-lg text-[11px] font-medium bg-green-500/10 border border-green-500/30 text-green-400 hover:bg-green-500/20 transition-colors"
                  >
                    ✓ Godkänn
                  </button>
                )}
                {payout.status === 'pending' && !isAdmin && (
                  <div className="flex-shrink-0 px-3 py-1.5 rounded-lg text-[11px] text-gray-600 bg-white/[0.03] border border-white/[0.06]">
                    🔒 Admin krävs
                  </div>
                )}
              </div>
...
export function PaymentProcessor() {
  ...
  return (
    <div className="space-y-4">
      {/* Entity context */}
      <div
        className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-medium w-fit"
        style={{
