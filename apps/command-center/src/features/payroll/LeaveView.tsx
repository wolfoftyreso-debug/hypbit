...
function SimpleCalendar({ month, year, employees }: { month: number; year: number; employees: Employee[] }) {
  ...
  return (
    <div className="bg-surface-raised border border-surface-border rounded-xl p-4">
      ...
      <div className="grid grid-cols-7 gap-px text-center text-xs text-gray-9000 mb-1">
          ...
          return (
            <div
              ...
              className={`relative flex flex-col items-center justify-start py-1 rounded text-xs min-h-[28px] ${
                isHoliday ? 'text-red-700' : isToday ? 'font-bold text-purple-300' : 'text-gray-9000'
...
export function LeaveView() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64 text-gray-9000">Laddar semesterdata...</div>
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-gray-9000 mt-0.5">25 semesterdagar/år (semesterlagen) · Röda dagar markerade i rött</p>
      </div>
      ...
      <div className="bg-surface-raised border border-surface-border rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-surface-border">
          ...
          <div className="flex gap-4 mt-2">
            <div className="flex items-center gap-1.5 text-xs text-gray-9000">
                  ...
                  <div className="mt-2 space-y-1">
                    {record.plannedLeave.map((pl, i) => (
                      <div key={i} className="flex items-center justify-between text-xs">
                        <span className="text-gray-9000">{pl.start} → {pl.end} ({pl.days} dagar)</span>
      ...
      <div>
        <h3 className="text-xs font-semibold text-gray-9000 uppercase tracking-wider mb-3">Kalendervy</h3>
          ...
          {SWEDISH_HOLIDAYS_2026.map(h => {
            ...
            return (
              <div key={h.date} className={`flex items-center gap-2 text-xs ${isPast ? 'opacity-40' : ''}`}>
                <span className="text-red-700 font-mono text-xs w-20 flex-shrink-0">
                  {h.date.slice(5)}
                </span>
