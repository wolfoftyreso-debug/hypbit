    ...
    <div className="my-6 overflow-x-auto">
      <div className="min-w-[600px]">
        ...
        <div className="flex items-center justify-center gap-2 mb-2">
          ...
          <ArrowRight size={16} className="text-[#8A8A9A] flex-shrink-0" />
        ...
        <div className="flex justify-center mb-2">
          <div className="w-px h-6 bg-[#EDE8DC] ml-[-2px]" style={{ marginLeft: 'calc(25% - 1px)' }} />
        </div>
...
function ArchNode({ label, sub, icon: Icon, highlight }: {
  ...
  return (
    <div className={`flex flex-col items-center gap-1 px-4 py-3 rounded border text-center min-w-[130px] ${
      highlight
        ? 'border-[#DDD5C5] bg-white'
        : 'border-[#DDD5C5] bg-white'
    }`}>
      <Icon size={18} className={highlight ? 'text-[#2C5F8A]' : 'text-[#8A8A9A]'} />
      <span className={`text-xs font-semibold tracking-wide ${highlight ? 'text-[#0A3D62]' : 'text-[#6B7280]'}`}>
        {label}
      </span>
      <span className="text-[10px] text-[#8A8A9A] leading-tight">{sub}</span>
    </div>
  )
}
...
function ArchNodeSmall({ label, icon: Icon }: { label: string; icon: React.ElementType }) {
  return (
    <div className="flex flex-col items-center gap-1 px-3 py-2 rounded border border-[#DDD5C5] bg-[#F5F0E8] text-center min-w-[100px]">
      <Icon size={14} className="text-[#8A8A9A]" />
      <span className="text-[10px] text-[#8A8A9A] leading-tight">{label}</span>
    </div>
  )
}
...
function ChapterAccordion({ chapters }: { chapters: Chapter[] }) {
  ...
  return (
    <div className="divide-y divide-[#DDD5C5] border border-[#DDD5C5] rounded">
      {chapters.map((ch) => {
        const Icon = ch.icon
        ...
        return (
          <div key={ch.id}>
            <button
              ...
              className="w-full flex items-center justify-between px-5 py-4 hover:bg-[#EDE8DC] transition-colors text-left"
            >
              <div className="flex items-center gap-3">
                <Icon size={16} className="text-[#8A8A9A] flex-shrink-0" />
                <span className="text-sm font-medium text-[#2C5F8A]">{ch.title}</span>
              </div>
              {isOpen
                ? <ChevronDown size={14} className="text-[#8A8A9A]" />
                : <ChevronRight size={14} className="text-[#8A8A9A]" />
              }
            </button>
            {isOpen && (
              <div className="px-5 pb-5 pt-1 text-sm text-[#8A8A9A] leading-relaxed space-y-3 border-t border-[#DDD5C5]">
                {ch.content}
              </div>
            )}
          </div>
...
const CHAPTERS: Chapter[] = [
  {
    ...
    content: (
      <div className="space-y-3">
        <p>OpenClaw är <strong className="text-[#6B7280]">runtime-miljön som kör Bernt</strong> — tänk på det som ett operativsystem för AI-agenten. Det är installerat på Eriks server (WSL2/Linux) och kör dygnet runt.</p>
        ...
        <p className="text-[#8A8A9A] text-xs pt-1">
        ...
        <ol className="space-y-2 ml-2 list-none">
          {[
            ...
            <li key={i} className="flex items-start gap-3">
              ...
              <div>
                <span className="text-[#6B7280] font-medium">{step}</span>
                <span className="text-[#8A8A9A]"> — {detail}</span>
              </div>
            </li>
          ))}
        </ol>
        <p className="text-[#8A8A9A] text-xs border-t border-[#DDD5C5] pt-3">
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          <div className="p-3 rounded border border-[#DDD5C5] bg-[#F5F0E8]">
            <div className="flex items-center gap-2 mb-1">
              <Database size={13} className="text-[#8A8A9A]" />
              <span className="text-[#6B7280] font-medium text-xs">MEMORY.md — Långtidsminne</span>
            </div>
          ...
          <div className="p-3 rounded border border-[#DDD5C5] bg-[#F5F0E8]">
            <div className="flex items-center gap-2 mb-1">
              <Database size={13} className="text-[#8A8A9A]" />
              <span className="text-[#6B7280] font-medium text-xs">memory/YYYY-MM-DD.md — Dagliga loggar</span>
            </div>
          ...
          <div className="p-3 rounded border border-[#DDD5C5] bg-[#F5F0E8]">
            <div className="flex items-center gap-2 mb-1">
              <Database size={13} className="text-[#8A8A9A]" />
              <span className="text-[#6B7280] font-medium text-xs">SOUL.md, USER.md, TOOLS.md</span>
            </div>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        ...
        <div className="overflow-hidden rounded border border-[#DDD5C5]">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-[#DDD5C5] bg-[#F5F0E8]">
                <th className="text-left px-3 py-2 text-[#8A8A9A] font-medium">Jobb</th>
                <th className="text-left px-3 py-2 text-[#8A8A9A] font-medium">Schema</th>
                <th className="text-left px-3 py-2 text-[#8A8A9A] font-medium">Vad det gör</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#DDD5C5]">
              {[
                ...
                <tr key={i} className="hover:bg-[#EDE8DC] transition-colors">
                  ...
                  <td className="px-3 py-2 text-[#8A8A9A]">{desc}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    content: (
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-start gap-3 p-3 rounded border border-[#DDD5C5]">
            <MessageSquare size={14} className="text-[#8A8A9A] mt-0.5 flex-shrink-0" />
            <div>
              <p className="text-[#6B7280] text-xs font-medium mb-1">Via Telegram</p>
              <p className="text-xs text-[#8A8A9A]">Håll inne mikrofon-ikonen i Telegram → skicka → OpenClaw tar emot och skickar till Whisper för transkribering → Bernt svarar.</p>
            </div>
          </div>
          <div className="flex items-start gap-3 p-3 rounded border border-[#DDD5C5]">
            <Mic size={14} className="text-[#8A8A9A] mt-0.5 flex-shrink-0" />
            <div>
              <p className="text-[#6B7280] text-xs font-medium mb-1">Via Wavult Mobile (VoiceButton)</p>
              <p className="text-xs text-[#8A8A9A]">Öppna Wavult-appen → håll inne röstknappen → pulsanimation visar inspelning → auto-skicka vid release → Bernt svarar i appen.</p>
            </div>
          </div>
          <div className="flex items-start gap-3 p-3 rounded border border-[#DDD5C5]">
            <Zap size={14} className="text-[#8A8A9A] mt-0.5 flex-shrink-0" />
            <div>
              <p className="text-[#6B7280] text-xs font-medium mb-1">Siri-genväg (iPhone)</p>
              <p className="text-xs text-[#8A8A9A]">"Hey Siri, Bernt" → öppnar Wavult Mobile direkt på röstläget via deep link <code className="text-[#8A8A9A]">wavult://voice</code>.</p>
            </div>
          </div>
        </div>
      </div>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <p className="text-[#6B7280] text-xs font-medium">Bernt kan:</p>
          <ul className="space-y-1.5 ml-2">
            {[
          ...
          <p className="text-[#6B7280] text-xs font-medium pt-2">Bernt kan INTE utan godkännande:</p>
          <ul className="space-y-1.5 ml-2">
            {[
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-12">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[10px] font-mono tracking-widest text-[#8A8A9A] border border-[#DDD5C5] rounded px-2 py-0.5 uppercase">
              Infrastructure
            </span>
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-[#0A3D62]">
            OpenClaw
          </h1>
          <p className="text-lg text-[#8A8A9A] font-light">
            The intelligence layer that powers Bernt.
          </p>
          <p className="text-sm text-[#8A8A9A] max-w-2xl leading-relaxed">
        ...
        {/* ── Status ── */}
        <div className="space-y-3">
          <h2 className="text-xs font-semibold tracking-widest text-[#8A8A9A] uppercase">
            System Status
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {STATUS_ITEMS.map((item) => (
              <div
                key={item.label}
                className="flex items-center gap-2.5 px-3 py-2.5 rounded border border-[#DDD5C5] bg-[#F5F0E8]"
              >
                <StatusDot status={item.status} />
                <span className="text-xs text-[#8A8A9A]">{item.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="rounded border border-[#DDD5C5] bg-[#F5F0E8] px-4 py-6">
            <ArchDiagram />
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="rounded border border-[#DDD5C5] overflow-hidden">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-[#DDD5C5] bg-[#F5F0E8]">
                  <th className="text-left px-4 py-3 text-[#8A8A9A] font-medium">Fras till Bernt</th>
                  <th className="text-left px-4 py-3 text-[#8A8A9A] font-medium">Vad som händer</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#DDD5C5]">
                {QUICK_REF.map((row, i) => (
                  <tr key={i} className="hover:bg-[#EDE8DC] transition-colors">
                    <td className="px-4 py-3 font-mono text-[#6B7280]">{row.phrase}</td>
                    <td className="px-4 py-3 text-[#8A8A9A]">{row.action}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* ── Footer ── */}
        <div className="border-t border-[#DDD5C5] pt-6 flex items-center justify-between">
          <span className="text-xs text-[#8A8A9A]">
            OpenClaw · github.com/openclaw/openclaw
          </span>
          <div className="flex items-center gap-2">
            <Cpu size={12} className="text-[#8A8A9A]" />
            <span className="text-xs text-[#8A8A9A]">Powered by Bernt</span>
          </div>
        </div>
