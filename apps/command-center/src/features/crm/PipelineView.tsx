    ...
    <div className="bg-surface-overlay border border-surface-border rounded-xl p-3.5 flex flex-col gap-2.5 group hover:border-gray-300 transition-colors">
      ...
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-gray-9000 mt-0.5">{prospect.contactName}</p>
        </div>
        <span
      ...
      <div className="text-base font-bold text-gray-900 tabular-nums">
        ...
        <span className="text-xs text-gray-9000 font-normal ml-1">/år</span>
      </div>

      {/* Meta row */}
      <div className="flex items-center gap-3 text-xs text-gray-9000">
      ...
      {/* Next step */}
      <p className="text-xs text-gray-9000 italic truncate" title={prospect.nextStep}>
        → {prospect.nextStep}
      </p>
      ...
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity mt-0.5">
        {prevStage && (
          <button
            ...
            className="text-xs px-2 py-0.5 rounded bg-surface-base text-gray-9000 hover:text-gray-900 transition-colors flex-1 truncate"
...
export function PipelineView() {
  ...
  return (
    <div className="space-y-5 max-w-full">
        ...
        {STAGE_ACTIVE.map(stage => {
          ...
          return (
            <div key={stage} className="flex flex-col gap-3 min-w-0">
              ...
              {cards.length > 0 && (
                <p className="text-xs text-gray-9000 px-1 tabular-nums">{formatSEK(stageValue)}</p>
              )}
              <div
          ...
          return (
            <div key={stage} className="flex flex-col gap-3 min-w-0">
              <div className="flex items-center justify-between px-1">
                ...
                <div className="flex items-center gap-2">
                  {cards.length > 0 && (
                    <span className="text-xs text-gray-9000 tabular-nums">{formatSEK(stageValue)}</span>
                  )}
                  <span
      ...
      <p className="text-xs text-gray-9000 pt-1 border-t border-surface-border">
