...
const ELKS_CONFIGURED = false // Mock: not configured
...
export function SMSView() {
  ...
  function sendSMS() {
    ...
    const newSMS: SMSLog = {
      ...
      message,
      status: ELKS_CONFIGURED ? 'pending' : 'failed',
      timestamp: new Date().toISOString(),
      provider: '46elks',
    }
    setLog(prev => [newSMS, ...prev])
    setMessage('')
    setRecipient('')
    setCustomNumber('')
    setShowForm(false)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-[11px] font-medium ${
          ELKS_CONFIGURED
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl border border-blue-500/20 bg-blue-500/10 text-blue-400 text-[11px] font-medium">
          <span className="h-2 w-2 rounded-full bg-blue-400" />
          Twilio: Fallback aktiv
        </div>
        <div className="ml-auto flex items-center gap-3">
          <span className="text-[11px] text-gray-500">{sentCount} skickade · {failedCount} misslyckade</span>
          <button
      ...
      {showForm && (
        <div className="bg-[#0D0F1A] rounded-xl border border-brand-accent/20 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-[12px] font-semibold text-white">Nytt SMS</h3>
            <button onClick={() => setShowForm(false)} className="text-gray-500 hover:text-gray-300">×</button>
          </div>
          <div>
            <label className="text-[10px] text-gray-500 font-mono uppercase tracking-wider mb-1 block">Mottagare</label>
            <select
              value={recipient}
          ...
          {recipient === 'custom' && (
            <div>
              <label className="text-[10px] text-gray-500 font-mono uppercase tracking-wider mb-1 block">Telefonnummer</label>
              <input
                value={customNumber}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-[#07080F] border border-white/[0.08] rounded-lg px-3 py-2 text-[12px] text-white placeholder-gray-600 focus:outline-none focus:border-brand-accent/50 resize-none"
            />
          </div>
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={sendSMS}
              className="px-4 py-1.5 rounded-lg text-[11px] font-medium bg-brand-accent text-white hover:bg-brand-accent/90 transition-colors"
            >
              Skicka →
            </button>
          </div>
      ...
      <div className="bg-[#0D0F1A] rounded-xl border border-white/[0.06] overflow-hidden">
        <div className="px-4 py-3 border-b border-white/[0.06]">
          <h3 className="text-[12px] font-semibold text-white">SMS-logg</h3>
        </div>
            ...
            <div key={sms.id} className="px-4 py-3 flex items-start gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[12px] font-medium text-gray-200">{sms.toName}</span>
                  <span className="text-[10px] text-gray-600 font-mono">{sms.to}</span>
                ...
                <p className="text-[11px] text-gray-400 truncate">{sms.message}</p>
                <div className="flex items-center gap-3 mt-1">
                  <span className="text-[10px] text-gray-600 font-mono">
                    {new Date(sms.timestamp).toLocaleString('sv-SE')}
                  </span>
                  <span className="text-[10px] text-gray-700">via {sms.provider}</span>
                  {sms.cost && <span className="text-[10px] text-gray-700">{sms.cost}</span>}
                </div>
              </div>
            </div>
          ))}
