    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-[#0A3D62]">Media Pipeline</span>
        </div>
        <button
          onClick={fetchMedia}
          className="flex items-center gap-1.5 text-xs text-[#8A8A9A] hover:text-[#6B7280] transition-colors"
        >
            ...
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${
                filterStatus === s
                  ...
                  : 'border-[#DDD5C5] text-[#8A8A9A] hover:border-[#DDD5C5] hover:text-[#6B7280]'
              }`}
            >
      ...
      <div className="bg-white border border-[#DDD5C5] rounded-xl overflow-hidden">
        {loading ? (
          <div className="p-8 flex items-center justify-center">
            <RefreshCw size={20} className="text-[#8A8A9A] animate-spin" />
          </div>
        ) : error ? (
          ...
          <div className="p-10 flex flex-col items-center gap-4 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-[#0A3D62] mb-1">Ingen media ännu</p>
              <p className="text-xs text-[#8A8A9A] max-w-xs">
            ...
            <div className="flex items-center gap-6 mt-2">
              <div className="text-center">
                <div className="flex items-center justify-center gap-1.5 mb-1">
                  ...
                  <span className="text-[10px] text-[#8A8A9A] font-mono uppercase">Upload</span>
                </div>
              </div>
              <div className="text-[#8A8A9A]">→</div>
              <div className="text-center">
                <div className="flex items-center justify-center gap-1.5 mb-1">
                  <Film size={12} className="text-yellow-400" />
                  <span className="text-[10px] text-[#8A8A9A] font-mono uppercase">Process</span>
                </div>
              </div>
              <div className="text-[#8A8A9A]">→</div>
              <div className="text-center">
                <div className="flex items-center justify-center gap-1.5 mb-1">
                  ...
                  <span className="text-[10px] text-[#8A8A9A] font-mono uppercase">Archive</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="divide-y divide-[#DDD5C5]">
            {filtered.map(file => (
              <div key={file.id} className="flex items-center gap-3 px-4 py-3">
                <div className="w-8 h-8 rounded bg-[#EDE8DC] flex items-center justify-center flex-shrink-0">
                  <Image size={14} className="text-[#8A8A9A]" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-mono text-[#2C5F8A] truncate">{file.name}</span>
                    <span className="text-[10px] text-[#8A8A9A]">{file.sizeMb.toFixed(1)} MB</span>
                  </div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[10px] text-[#8A8A9A]">Mission: {file.missionId}</span>
                    {file.aiTags && file.aiTags.slice(0, 3).map(t => (
                      <span key={t} className="text-[9px] px-1.5 py-0.5 rounded bg-[#EDE8DC] text-[#8A8A9A]">{t}</span>
                    ))}
                  </div>
                  {(file.status === 'uploading' || file.status === 'processing') && (
                    <div className="mt-1 h-1 rounded-full bg-[#DDD5C5] overflow-hidden">
                      <div
